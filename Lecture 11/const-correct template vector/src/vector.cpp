...
void vector<T>::debug() const {
    ...
    std::cout << std::endl;
    // need const_iterators: std::copy(begin(), end(), std::ostream_iterator<int>(std::cout, " "));
    // you could also do this: std::copy(const_cast<iterator>(begin(), const_cast<iterator>(end()), ...));
    std::cout << '\n';
}
